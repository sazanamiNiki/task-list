...
- **並行アクセス対応**: ファイルロック機構により複数のClaude Codeセッションからの同時アクセスを安全に処理
...
複数のClaude Codeセッション（複数のMCPサーバープロセス）が同時に動作する場合でも、ファイルロック機構により安全にタスクデータを操作できます。

### 動作の仕組み

1. **ロックの取得**: 各MCPサーバープロセスはtasks.jsonへの書き込み前にファイルロックを取得
2. **待機**: 他のプロセスがロックを保持している場合は自動的に待機（最大10回リトライ、待機時間50ms〜1秒）
3. **処理の実行**: ロックを取得後、ファイルの読み込み→変更→書き込みをアトミックに実行
4. **ロックの解放**: 処理完了後、即座にロックを解放

### 利点

- **データ損失の防止**: 複数のセッションで同時にタスクを追加・更新しても、すべての変更が保持されます
- **自動リトライ**: 一時的なロックの競合は自動的に解決されます
- **透過的な処理**: ユーザーやクライアントは待機を意識する必要がありません

### 使用例

```bash
# ターミナル1: プロジェクトAのClaude Codeセッション
# tasks.jsonにproject-aのタスクを追加

# ターミナル2: プロジェクトBのClaude Codeセッション（同時実行）
# tasks.jsonにproject-bのタスクを追加

# → 両方の変更が安全にtasks.jsonに反映されます
```
